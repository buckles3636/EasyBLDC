...
typedef struct
{
  ...
  uint16_t hICLUnderVoltageThreshold;    /**< @brief Under voltage threshold under which ICL is active */
} ICL_Handle_t;
...
void ICL_Init(ICL_Handle_t *pHandle, DOUT_handle_t *pDOUT);

/* Executes the Inrush Current Limiter state machine */
ICL_State_t ICL_Exec(ICL_Handle_t *pHandle, uint16_t Vbus);
...
/**
  ...
  */
inline static void ICL_SetVoltageThreshold(ICL_Handle_t *pHandle, uint16_t hVoltageThreshold)
{
  pHandle->hICLVoltageThreshold = hVoltageThreshold;
}
