...
/**
  ...
  */
__weak void ICL_Init(ICL_Handle_t *pHandle,  DOUT_handle_t *pDOUT)
{
  pHandle->pDOUT = pDOUT;
...
/**
  ...
  * @param  pHandle handler of the current instance of the ICL component
  * @param  Vbus Vbus measurement expressed in Volt 
  * @retval ICLState_t returns the current ICL state machine
  */
__weak ICL_State_t ICL_Exec(ICL_Handle_t *pHandle, uint16_t Vbus)
{
  /* ICL actions.*/
  switch (pHandle->ICLstate)
  {
    ...
    case ICL_ACTIVE:
    {
      ...
      if (pHandle->hICLTicksCounter == 0u)
      {
        if ( Vbus > pHandle->hICLVoltageThreshold){
    ...
    case ICL_INACTIVE:
    {
      ...
      if ( Vbus < pHandle->hICLUnderVoltageThreshold)
      {
